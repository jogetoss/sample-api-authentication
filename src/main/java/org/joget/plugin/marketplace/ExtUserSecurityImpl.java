...
public class ExtUserSecurityImpl extends UserSecurityImpl implements HiddenPlugin {
    ...
    public String getName() {
        return "Sample API Auth User Security";
    }

    @Override
    public String getDescription() {
        return "User Security for Sample API Auth";
    }
    ...
    public String getLoginFormFooter() {
        ...
        content += "   <script>"
                ...
                + "        $('#externalLogin').click(() => {\n"
                + "            var form = $('#loginForm');\n"
                + "            $('#j_username', form).attr('name', 'sampleUsername');\n"
                + "            $('#j_password', form).attr('name', 'samplePassword');\n"
                + "            form.attr('action', '/jw/web/json/plugin/org.joget.plugin.marketplace.SampleAPIDirectoryManager/service')\n"
                + "            .submit();\n"
                + "        })\n"
                + "    });\n"
                + "    </script>";
        ...
        content += "<button id=\"externalLogin\"><i id=\"icon\" class=\"" + dmImpl.getPropertyString("buttonIcon") + "\"></i>" + dmImpl.getPropertyString("buttonText") + "</button>";
